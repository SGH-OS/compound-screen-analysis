...
```{r}
expr <- read.csv("compound screen(HOS).csv",header = T) #need to change the .xlsx file to .csv file
...
ggsave(file = "scatter plot(HOS).pdf",height = 5,width = 6)
```


```{r}
cor <- read.csv("compound screen(HOS).csv",header = T)
library(dplyr)
cor <- cor %>%
    group_by(Compound_Name) %>%
    summarise_all(mean)
...
ggsave(file = "ranked scatter plot(HOS).pdf",height = 5,width = 6)
